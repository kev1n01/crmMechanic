    ...
    <x-form method="save">
        <x-modal-dialog :id="$idModal" title="{{ $nameModal }}">
            <x-slot name="body">
                ...
                <x-input.input-tooltip-error class="col-12" name="editing.name" label="Nombre de Marca" type="text"
...
@push('js')
    <script>
        window.addEventListener('close-modal-brand-vehicle', event => {
            $('#brandVehicleModal').modal('hide');
        });
        window.addEventListener('open-modal-brand-vehicle', event => {
