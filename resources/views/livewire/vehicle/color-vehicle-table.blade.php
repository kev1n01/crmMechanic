                        ...
                        <x-table>
                            ...
                            <x-slot name="body">
                                @forelse ($colors as $color)
                                    <x-table.row wire:key="row-{{ $color->id }}" wire:loading.class="bg-light"
                                        wire:target="search">
                                        <x-table.cell>
                                            <x-input.check-input name="selected" value="{{ $color->id }}" />
                                        </x-table.cell>
                                        <x-table.cell>{{ $color->id }}</x-table.cell>

                                        <x-table.cell>{{ $color->name }}</x-table.cell>

                                        <x-table.cell>{{ \Carbon\Carbon::parse($color->created_at)->format('d-m-Y') }}
                                        </x-table.cell>


                                        <x-table.cell>

                                            <a class="action-icon" wire:click="edit({{ $color->id }})">
                                                <i class="mdi mdi-square-edit-outline"></i> </a>
                                            <a class="action-icon" onclick="Confirm({{ $color->id }}, 'delete')"><i
    ...
    <x-form method="save">
        <x-modal-dialog :id="$idModal" title="{{ $nameModal }}">
            <x-slot name="body">
                ...
                <x-input.input-group>
                    <x-input.input-group>
                        <x-input.input-tooltip-error class="col-12" name="editing.name" label="Nombre del color"
