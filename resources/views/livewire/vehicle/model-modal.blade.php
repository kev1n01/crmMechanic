    ...
    <x-form method="save">
        <x-modal-dialog :id="$idModal" title="{{ $nameModal }}">
            <x-slot name="body">
                ...
                <x-input.input-tooltip-error class="col-12" name="editing.name" label="Nombre del modelo" type="text"
...
@push('js')
    <script>
        window.addEventListener('close-modal-model-vehicle', event => {
            $('#modelVehicleModal').modal('hide');
        });
        window.addEventListener('open-modal-model-vehicle', event => {
