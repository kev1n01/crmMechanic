...
class RequestProvider extends FormRequest
{
     ...
     */
    public function rules($provider = null)
    {
        return [
            'editing.name' => ['required', 'min:4', 'max:100', Rule::unique('providers', 'name')->ignore($provider)],
            'editing.phone' => ['required', 'min:9', 'max:9', Rule::unique('providers', 'phone')->ignore($provider)],
            'editing.address' => ['min:5', 'max:100', Rule::unique('providers', 'address')->ignore($provider)],
